...
function Covers({ activeBookIDs }) {
  ...
  // a width is determined and avoids the weird animation effect
  if (gridWidth === 0) {
    activeBookIDs = [];
  }
  ...
  const booksWithPositionAndDimensions = activeBookIDs.map((bookID) => {
    // Looks up the book info from the book.json file
